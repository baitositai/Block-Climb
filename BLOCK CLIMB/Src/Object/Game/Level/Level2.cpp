...
{	
	...
	player_->SetParam(PlayerBase::MAX_MOVE_SPEED, PlayerBase::MAX_JUMP_POW * 1.2);
...
#pragma region 各パラメーター初期化用変数
	...
	//ステージサイズ
	stageSize_ = { groundSize_.x,
					fieldSize_.y + 3 };

	float fieldY = fieldSize_.y;

	//プレイヤーの初期座標
	firstPlayerPos_ = { Application::SCREEN_SIZE_X / 2 - 50,
	...
	game_Draw_Center_ = { Application::SCREEN_SIZE_X / 2				//スクリーン画面の中心から
						- stageSize_.x * BlockBase::BLOCK_SIZE_X / 2	//ゲーム画面の半分引く
						+ BlockBase::BLOCK_SIZE_X / 2,					//0.5ブロックの半分足す
							0 };
#pragma endregion
