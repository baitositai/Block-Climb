...
void Text::SetFont()
{
	buttonUIFont_ = CreateFontToHandle("ベストテンDOT", EXPLANATION_FONT_SIZE, 3);
	normalFont_ = CreateFontToHandle("ベストテンDOT", FONT_SIZE, 3);
	selectFont_ = CreateFontToHandle("ベストテンDOT", SELECT_FONT_SIZE, 3);
	explanationFont_ = CreateFontToHandle("ベストテンDOT", EXPLANATION_FONT_SIZE, 3);
}
...
void Text::BottanUIDraw()
{
	SetDrawBlendMode(DX_BLENDMODE_ALPHA, 150);
	int length = buttonUIText_.size() * EXPLANATION_FONT_SIZE / 2 + 40;
	Vector2 start{ Application::SCREEN_SIZE_X - length,0 };
	Vector2 finish{ Application::SCREEN_SIZE_X, EXPLANATION_FONT_SIZE + 8};
	...
	DrawStringToHandle(start.x, start.y, buttonUIText_.c_str(), 0xffffff, buttonUIFont_);
}

void Text::NewRecodeDraw()
{
	int border = 3;
	Vector2 boxPos = {Application::SCREEN_SIZE_X / 2 + 150 ,150 };
	Vector2 boxSize = { 330, 120 };
	...
	DrawFormatStringToHandle(interval + boxPos.x, 
		boxPos.y+ EXPLANATION_FONT_SIZE, 
...
void Text::MenuText()
{
	...
	{
		int fontIntevel = FONT_SIZE * i * 1.5;
		int color = 0xffffff;
...
Vector2 Text::TextScroll(Vector2 pos ,int length)
{
...
int Text::GetMenuTextSize(int i)
{
	return menuText_[i].size();
}
