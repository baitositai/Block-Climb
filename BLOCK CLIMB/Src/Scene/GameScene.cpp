...
#include "../Object/Game/SceneSelect.h"
...
void GameScene::Init(void)
{
	...
	//BGMの再生
	SoundManager::GetInstance().PlayBgmSound(bgm_, 70, true);
}
	...
	else
	{
			...
			(state_ == STATE::GAME || state_ == STATE::PAUSE))
		{
			...
			{ 
				...
				snd.PlayBgmSound(bgm_, 70, false);
				state_ = STATE::GAME; 
			}
			//一時停止中にもう一度押した場合
			else 
...
void GameScene::LevelLoad()
{
	...
	case Data::LEVEL::MARASON:
		...
		randNum = rand() % 4;
		if (randNum == 0) { bgm_ = SoundManager::BGM::GAME1; }
		else if (randNum == 1) { bgm_ = SoundManager::BGM::GAME2; }
		else if (randNum == 2) { bgm_ = SoundManager::BGM::GAME3; }
		else if (randNum == 3) { bgm_ = SoundManager::BGM::GAME4; }
		break;
...
void GameScene::ChangeScene(SceneManager::SCENE_ID scene)
{
	//シーンを変える
...
{
	SoundManager::GetInstance().PlayBgmSound(bgm_, 70, false);
}
